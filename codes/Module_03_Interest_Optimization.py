...
# Once you have printed your saved audiences, filter only the audiences you are interested for the optimization
# Just keep in mind that the higher the number of audiences, rate limits will be reached faster

# Filter by name only those audiences that you want to optimize on interests
audiences_list = ['Test_OptiMate', # SET YOUR OWN AUDIENCE NAME
                 ]
...
for interests in append_audiences['interest_id']:
    ...
    append_audiences.loc[count, 'ind_reach'] = int(reach_estimate[0]['users_lower_bound'])
...
for audience in append_audiences['audience_name'].unique():
    ...
    for i in range(0, len(interest_list_ids)):
        ...
        for interests in df_interests_order['interest_id']:
            ...
            temp_df.loc[count_01, 'joint_reach'] = reach_estimate[0]['users_lower_bound']
...
df_interest_optim[:][df_interest_optim[:]['audience_name'] == audience]
...
for audience in df_interest_optim['audience_name'].unique():
    ...
    #plt.title('Optimized Interests: Audience_01')
